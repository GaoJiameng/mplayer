   ...
   if(buf !=NULL)
    {
        ...
        {
            time = atof((char *)buf_src[1]);
            minute = ((int)time*10)/60/10;
            second = ((int)time*10)%60/10;
            sprintf(nowtime, "%02d:%02d", minute , second);
            printf("nowtime = %s\n",buf_src[0]);
            //gtk_label_set_text(GTK_LABEL(w.label_time) , nowtime);
            usleep(1000*1000);
        }
    }
}
...
void length_dispose(char buf[])   //获得歌的总长度
{
    ...
    if(buf !=NULL)
    {
        ...
        if(strcmp(buf_src[0],"ANS_LENGTH")==0)
        {
            ...
            minute = ((int)time*10)/60/10;
            second = ((int)time*10)%60/10;
            sprintf(fulltime, "%02d:%02d", minute , second);
            printf("fulltime = %s\n",buf_src[0]);
            //gtk_label_set_text(GTK_LABEL(w.label_time_01) , fulltime);
            usleep(1000*1000);
        }
    }
}
